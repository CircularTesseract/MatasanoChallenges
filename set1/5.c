...
char* asciiToBinary(char* string){
	char* result = malloc(strlen(string)*8+1);
	result[strlen(string)*8]='\0'; 

	int i=0; 
	for(i; i<strlen(string)*8;i+=8){
		char* temp = charToBinary(string[i/8]); 
		int j=0; 
...
void main(){
	char* counter ="ICE"; 
	char* cypherText = "Burning 'em, if you ain't quick and nimble\nI go crazy when I hear a cymbal"; 
	char* result= xorCypher(asciiToBinary(cypherText),asciiToBinary(counter)); 
	printf("%s\n",result );
}